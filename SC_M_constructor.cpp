	...
	_left_top(c_RENDER_WIDTH/5, c_RENDER_HEIGHT/5)
{
	...
	_menu->Add_option(new Option("Rozpocznij grê", [this]() {
		_game_commands->switch_scene.Execute(new RaceScene(_game_commands, _menu->Get_option_value(1), _menu->Get_option_value(2)));
		}));
	_menu->Add_option(new OptionWithValue("Liczba graczy", 2, 1, 4));
